...
class Camera:
    def __init__(self, camera_index: int = 0):
        ...
        self.current_settings = None  # Store current settings
        
        # Try to initialize
        self.initialize()
        
    @staticmethod
    def get_available_cameras() -> Dict[int, Dict[str, any]]:
        ...
        for backend in backends:
            ...
            while True:
                try:
                    ...
                    # Store camera details in a dictionary
                    available_cameras[index] = {
                        'name': name,
                        'device_id': f'index_{index}',  # or any other identifier you want
                        'camera_type': 'USB',  # or whatever type you want to assign
                        'resolution': "1280x720",  # default resolution
                        'fps': 30,  # default fps
                        'brightness': 50,  # default brightness
                        'exposure': 50,  # default exposure
                        'supported_resolutions': {"640x480": [30, 60], "1280x720": [30, 60]}
                    }
                    cap.release()
                    index += 1
        ...
        if not available_cameras:
            ...
            return {0: {"name": "Default Camera"}}  # Return a default camera if none detected
    ...
    def update_settings(self, new_settings):
        if self.current_settings != new_settings:
            ...
            self.change_fps(new_settings['fps'])  # Example of updating FPS
            self.cap.set(cv2.CAP_PROP_BRIGHTNESS, new_settings['brightness'])
            self.cap.set(cv2.CAP_PROP_EXPOSURE, new_settings['exposure'])
            # Add logic to handle zoom if supported by the camera
            if hasattr(cv2, 'CAP_PROP_ZOOM'):
                self.cap.set(cv2.CAP_PROP_ZOOM, new_settings.get('zoom', 1.0))
            print("Updating camera settings...")
        else:
            print("No changes in camera settings; skipping update.") 
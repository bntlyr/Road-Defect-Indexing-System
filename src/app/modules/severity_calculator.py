...
from src.app.modules import fuzzy_logic  # <-- Add this import
from src.app.modules import random_forest  # Add this import
from src.app.modules import traffic_getter  # <-- Add this import
...
class SeverityCalculator:
    ...
    def calculate_severity(self, 
        ...
        """
        Calculate severity metrics for detected defects and call fuzzy logic for severity percentage.
        ...
        Returns:
            Tuple of (severity_level, real_world_area_m2, total_bbox_area, fuzzy_severity_percent)
        """
        if not detections:
        ...
        return severity_level, real_world_area, total_bbox_pixels, fuzzy_severity
    ...
    def process_image(self,
        ...
        try:
            ...
            fuzzy_severity = fuzzy_logic.calculate_severity_percentage(
                self.pixel_size_y * gray_img.shape[0] * 10,  # defect_length_cm (whole image)
                self.pixel_size_x * gray_img.shape[1] * 10,  # defect_width_cm (whole image)
                avg_defect_ratio, defect_type='pothole',  # Placeholder for defect type
            )

            # Get traffic value using traffic_getter if GPS is available
            if gps_loc:
                _, traffic_value = traffic_getter.get_road_and_traffic_from_gps(gps_loc[0], gps_loc[1])
            else:
                traffic_value = 50000

            # Call random forest model for repair probability using traffic value
            repair_probability = random_forest.predict_repair_probability(
                traffic_value,
                avg_defect_ratio,
                fuzzy_severity
            )
            ...
            output_metadata = {
                ...
                'RepairProbability': repair_probability  # Now 1 or 0
            }

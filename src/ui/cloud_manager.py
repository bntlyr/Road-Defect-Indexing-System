...
class CloudManagerDialog(QDialog):
    ...
    def setup_ui(self):
        ...
        # Create a ComboBox for view selection
        ...
        self.upload_btn.clicked.connect(self.upload_selected_images)
        
        # Add Refresh button
        self.refresh_btn = QPushButton("Refresh")
        self.refresh_btn.clicked.connect(self.refresh_file_list)  # Connect to refresh method
        control_layout.addWidget(self.refresh_btn)  # Add the refresh button to the layout
        ...
        if self.is_cloud_view:
            ...
            for detection in detections:
                if detection.lower().endswith(('.jpg', '.jpeg', '.png')):
                    item = QListWidgetItem(detection)
    ...
    def upload_selected_images(self):
        ...
        if not selected_items:
            QMessageBox.warning(self, "No Selection", "Please select files to upload")
            return

        # Create progress dialog
        progress = QProgressDialog("Uploading files...", "Cancel", 0, len(selected_items), self)
        progress.setWindowModality(Qt.WindowModal)
        progress.setWindowTitle("Upload Progress")

        success_count = 0
        error_count = 0
        metadata_success_count = 0  # Count for successfully uploaded metadata
        ...
        for i, item in enumerate(selected_items):
            ...
            progress.setValue(i)
            progress.setLabelText(f"Uploading {item.text()}...")

            try:
                # Prepare the file path with the correct separator
                filename = item.text()  # Get the original filename
                file_path = os.path.join(self.local_path, filename).replace('\\', '/')  # Normalize path
                metadata_filename = f"{filename.rsplit('.', 1)[0]}_metadata.json"  # Construct metadata filename
                metadata_path = os.path.join(self.local_path, metadata_filename).replace('\\', '/')  # Normalize path
                ...
                if not success:
                    error_count += 1
                    print(f"Warning: Could not encode image {filename}")
                    continue
                ...
                # Upload to cloud, ensuring the path is set correctly
                if self.cloud_storage and self.cloud_storage.is_initialized:
                    upload_path = f"{filename}"  # Set to detections/ directory directly

                    # Upload the image
                    success = self.cloud_storage.upload_detection(image_bytes, metadata, upload_path)
                    if success:
                        success_count += 1
                        print(f"Uploaded detection: {upload_path}")
                    else:
                        error_count += 1
                ...
                if metadata:
                    metadata_upload_path = f"{filename.rsplit('.', 1)[0]}_metadata.json"  # Construct metadata upload path
                    success = self.cloud_storage.upload_metadata(metadata_upload_path, metadata)  # Hypothetical method to upload metadata
                    if success:
                        metadata_success_count += 1  # Increment metadata success count
                        print(f"Uploaded metadata: {metadata_upload_path}")
                    else:
                        error_count += 1
                        print(f"Error uploading metadata for {filename}")
        ...
        # Show results
        result_message = f"Upload Complete:\n"
        result_message += f"Successfully uploaded: {success_count} image(s)\n"
        result_message += f"Successfully uploaded: {metadata_success_count} metadata file(s)\n"
        if error_count > 0:
            result_message += f"Failed to upload: {error_count} file(s)\n"
        
        # Add total time taken to the message
        result_message += f"Total time taken: {total_time:.2f} seconds for {len(selected_items)} file(s) and {metadata_success_count} metadata file(s)"

        QMessageBox.information(self, "Upload Complete", result_message)

    def upload_metadata(self, path, metadata):
        """Uploads metadata to the cloud storage."""
        try:
            # Convert metadata to JSON string if necessary
            metadata_json = json.dumps(metadata)
            # Upload the metadata JSON string to the specified path
            # This is a placeholder for the actual upload logic
            # e.g., self.cloud_service.upload(path, metadata_json)
    ...
    def delete_selected_cloud_files(self):
        """Delete selected files from the cloud and their associated metadata."""
        selected_files = self.get_selected_cloud_files()
        ...
        for filename in selected_files:
            try:
                cloud_file_path = self.get_cloud_file_path(filename)  # Implement this method to get the cloud file path
                if cloud_file_path:
                    self.delete_file_from_cloud(cloud_file_path)  # Implement this method to delete from cloud

                # Delete the associated metadata.json file
                metadata_file_path = os.path.splitext(cloud_file_path)[0] + '_metadata.json'
                if os.path.exists(metadata_file_path):
                    os.remove(metadata_file_path)
            except Exception as e:
                QMessageBox.warning(self, "Error", f"Failed to delete {filename}: {str(e)}")

        self.refresh_cloud_file_list()  # Refresh the cloud file list after deletion
        QMessageBox.information(self, "Success", f"Successfully deleted {len(selected_files)} file(s) from the cloud")

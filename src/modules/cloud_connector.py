...
class CloudStorage:
    ...
    def upload_detection(self, image_data: bytes, metadata: Dict[str, Any], filename: str) -> bool:
        ...
        try:
            ...
            blob_name = filename
            if self.settings['folder_path'].rstrip('/'):
                blob_name = os.path.join(self.settings['folder_path'].rstrip('/'), filename).replace('\\', '/')
    ...
    def upload_metadata(self, path, metadata):
        ...
        try:
            ...
            # Create blob name with folder path if specified
            if self.settings['folder_path'].rstrip('/'):
                path = os.path.join(self.settings['folder_path'].rstrip('/'), path).replace('\\', '/')
            ...
            self.logger.info(f"Uploaded metadata: {path}")
            return True
            
        except Exception as e:
            self.logger.error(f"Error uploading metadata {path}: {e}")
            return False
